...
void add_entity (simState_t *state)
{
...
#endif // ifdef PLUG
{
    ...
    while (!WindowShouldClose ()) // Detect window close button or ESC key
    {
        ...
        else if (IsKeyPressed (KEY_P) && state->n_entities < MAX_ENTITIES)
        {
            ...
            {
                for (int i = 0; i < 10 && state->n_entities < MAX_ENTITIES; i++)
                {
                    add_entity (state);
                }
            }
        ...
        for (int i = 0; i < state->n_entities; i++)
        {
            ...
            Vector2 p1 =
                (Vector2){ent->pos.x + direction.x * ent->size.x / 2,
                          ent->pos.y + direction.y * ent->size.x / 2}; // Tip of the arrow (length)
            ...
            Vector2 p3 =
                (Vector2){ent->pos.x + direction.y * ent->size.y / 2,
                          ent->pos.y - direction.x * ent->size.y / 2}; // The other side of the base
        ...
        if (showEnergy)
        {
            ...
            DrawText (TextFormat ("Sim energy : %.0f", sqrtf (sumSqr)), GetScreenWidth () - 180, 50,
                      20, RED);
        }
        if (showRadii)
        {
        ...
        if (showCount)
        {
            DrawText (TextFormat ("Entity count : %d", state->n_entities), GetScreenWidth () - 200,
                      GetScreenHeight () - 30, 20, BLUE);
        }
        if (showHelp)
        {
            ...
            DrawText (TextFormat ("O - Add 10 entities (max %d)", MAX_ENTITIES), 10, 130, 20, BLUE);
